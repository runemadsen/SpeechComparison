class TotalWord extends DisplayObject
{
   ...
   void update()
   {
      if(_time <= _duration)   
      {
         if(_curDelay >= _startDelay || _reset)
         {
   ...
   void drawArcs()
   {
      for(int i = 0; i < _speekers.length; i++)
      {
         ...
         {
            stroke(_speekers[i].getLegendColor(), 0);
         }
   ...
   int getHighestNum()
   {
      return getHighestTotal();
      //return getHighestSingle();
   }
   
   /* Returns all the wordcounts added
   ___________________________________________________________________ */
   ...
   /* Returns the highest single wordcount
   ___________________________________________________________________ */
   
   int getHighestSingle()
   {
      int highestVal = 0;
